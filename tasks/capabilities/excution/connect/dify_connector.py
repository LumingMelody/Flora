...
class DifyConnector(BaseConnector):
    ...
    def _resolve_param(self, key: str, runtime_params: Dict[str, Any]) -> Any:
        ...
        try:
            ...
            if key == "api_key" and DIFY_API_KEY:
                return DIFY_API_KEY
            if key == "base_url" and DIFY_BASE_URL:
    ...
    def execute(self, inputs: Dict[str, Any], params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                from env import DIFY_RESPONSE_MODE
                response_mode = DIFY_RESPONSE_MODE or "blocking"
            except Exception:
            ...
            try:
                ...
                if "application/json" in content_type:
                    ...
                    # Try to parse SSE-style responses
                    ...
                    for line in text.splitlines():
                        if line.startswith("data:"):
                            ...
                            if payload_line and payload_line != "[DONE]":
                                last_data = payload_line
                    if last_data:
                        try:
                            result_data = json.loads(last_data)
                        except Exception:
                            result_data = None
                    if result_data is None:
                        return {
                            "status": "ERROR",
                            "error": f"Unexpected Dify response (content-type={content_type})",
                            "raw": text[:500],
                        }
